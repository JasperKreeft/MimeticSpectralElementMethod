...
stress_x_2 = +1*[ stress_x_y(nn*(nn-1)+(1:nn),7) ; stress_x_y(nn*(nn-1)+(1:nn),9) ];
stress_x_3 = +1*stress_x_x(nn:nn:nn^2,3); %stress_x_x(nn:nn:nn^2,9);
stress_x_4 = nx_4.*[ stress_x_x(nn:nn:nn^2,8) ; stress_x_x(1:nn,9) ]+ny_4.*[ stress_x_y(nn:nn:nn^2,8) ; stress_x_y(1:nn,9) ];

Int_x_1 = sum(wp'.*stress_x_1*0.75/2);
Int_x_2 = sum([wp wp]'.*stress_x_2*0.75/2);
Int_x_3 = sum(wp'.*stress_x_3*0.25/2);
Int_x_4 = sum([wp wp]'.*stress_x_4*(pi/4)/2);

Int_x = 2*(Int_x_1+Int_x_2+Int_x_3+Int_x_4); % 2x because of symmetry
...
stress_y_1 = -1*stress_y_x(1:nn:nn^2,7); %stress_y_x(1:nn:nn^2,7);
stress_y_2 = +1*[ stress_y_y(nn*(nn-1)+(1:nn),7) ; stress_y_y(nn*(nn-1)+(1:nn),9) ];
stress_y_3 = +1*stress_y_x(nn:nn:nn^2,3); %stress_y_x(nn:nn:nn^2,9);
stress_y_4 = nx_4.*[ stress_y_x(nn:nn:nn^2,8) ; stress_y_x(1:nn,9) ]+ny_4.*[ stress_y_y(nn:nn:nn^2,8) ; stress_y_y(1:nn,9) ];

Int_y_1 = 0; % v=0  sum(wp'.*stress_y_1*0.75/2);
...
for i=1:numElements
    ...
    stress_y_yp = reshape(vvy(:,i),nn,nn);%stress_y_y
