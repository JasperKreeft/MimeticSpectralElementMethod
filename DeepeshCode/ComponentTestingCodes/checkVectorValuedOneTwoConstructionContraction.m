function globalError = checkVectorValuedOneTwoConstructionContraction(p,n,xBound,yBound,velocity,momentum,pressure,pressureForce,momentumContraction,dMomConAnalytical,plotImages,periodic)
...
% Revision 1.0 $5/3/2012$
    ...
    pressureConstructionDiscrete = ConstructVectorPressureForce(n, g11, g12, g22, p, orientation,periodic);
    ...
    momentumConstructionDiscrete = ConstructVectorMomentum(p,orientation);
    ...
    momentumConstructionXi = zeros(nMomXi,nOne);
    momentumConstructionEta = zeros(nMomEta,nOne);
...
%                  reshape(momentumConstructionDiscrete.EtaE(:),p*(p+1),2*p*(p+1))];
        ...
        momentumConstructionXi(globalNumMomentum.Xi(element,:),globalNumOne(element,:)) = ...
            momentumConstructionXi(globalNumMomentum.Xi(element,:),globalNumOne(element,:)) + ...
                reshape(momentumConstructionDiscrete.Xi(:),p*(p+1),2*p*(p+1));
        % momentum in Eta finite-volumes
        momentumConstructionEta(globalNumMomentum.Eta(element,:),globalNumOne(element,:)) = ...
            momentumConstructionEta(globalNumMomentum.Eta(element,:),globalNumOne(element,:)) + ...
                reshape(momentumConstructionDiscrete.Eta(:),p*(p+1),2*p*(p+1));
    end
