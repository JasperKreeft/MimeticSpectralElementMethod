function StokesTest2D(n,p,xBound,yBound,velocity, momFlux, boundaryVel,plotImages)
...
% Revision 1.0 $5/3/2012$
    ...
    figurePressure = 7;
    figureVorticity = 8;
    figureNumberMomentum = [9 10];
    figureNumberMomentumError = 11;
    ...
    for element = 1:nElements
        ...
        indCXiG = [indCXiG;globalNumVectorOne.Xi(element,c1)'];
        valRCXiG = [valRCXiG;v1];
        
        indREtaG = [indREtaG;globalNumMomentum.EtaG(element,r2)'];
        indCEtaG = [indCEtaG;globalNumVectorOne.Eta(element,c2)'];
    ...
    CD21.XiG = sparse(double(indRXiG),double(indCXiG),valRCXiG,nMomXiG,nOneXi);
    CD21.EtaG = sparse(double(indREtaG),double(indCEtaG),valRCEtaG,nMomEtaG,nOneEta);
    CD21.Xi = sparse(double(indRXiG),double(indCXiG),valRCXiG,nMomXi,nOneXi);
    CD21.Eta = sparse(double(indREtaG),double(indCEtaG),valRCEtaG,nMomEta,nOneEta);
    ...
    %% Direct check of matrices
    
    momFluxEtaV = CD21.Eta*(CDStar12.RHS.Eta\((CDStar12.LHS.Eta + CDStar12.LHSBoundaryU.Eta)*momentumConstructionDiscrete.Eta*velocitiesDiscreteV + CDStar12.LHSBoundaryK.Eta));
    momFluxXiV = CD21.Xi*(CDStar12.RHS.Xi\((CDStar12.LHS.Xi + CDStar12.LHSBoundaryU.Xi)*momentumConstructionDiscrete.Xi*velocitiesDiscreteV + CDStar12.LHSBoundaryK.Xi));
    momFluxEta = momFluxEtaV(globalNumMomentum.Eta');
    momFluxXi = momFluxXiV(globalNumMomentum.Xi');
    momFluxDiscrete = [momFluxXi;momFluxEta];
    
    PlotVectorMomentum2D(momFluxDiscrete,dPhiXdXi,dPhiXdEta,dPhiYdXi,dPhiYdEta,g,phi,xBound,yBound,nReconstruction,figureNumberMomentum,orientation);
    globalError.vecTwoMomentum = L2ErrorVectorMomentum2D(momFluxDiscrete,momFlux,dPhiXdXi,dPhiXdEta,dPhiYdXi,dPhiYdEta,g11,g22,g12,g,phi,xBound,yBound,nReconstruction,figureNumberMomentumError,orientation);
    ...
    E21 = CDStar12.RHS.Xi;
    E22 = -(CDStar12.LHS.Xi + CDStar12.LHSBoundaryU.Xi)*momentumConstructionDiscrete.Xi;
    E23 = spalloc(nOneXi,nOneEta,1);
    E24 = spalloc(nOneXi,nPres,1);
    
    % Co-covariant derivative - Eta momentum
    E31 = CDStar12.RHS.Eta;
    E32 = spalloc(nOneXi,nOneXi,1);
    E33 = -(CDStar12.LHS.Eta + CDStar12.LHSBoundaryU.Eta)*momentumConstructionDiscrete.Eta;
