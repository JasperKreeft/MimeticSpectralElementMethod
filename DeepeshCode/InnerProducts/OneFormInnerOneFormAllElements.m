function innerProd = OneFormInnerOneFormAllElements(p,g11,g12,g22,g,nodesType,sparseFlag,varargin)
...
%     end
    ...
    for element = 1:nElements
        ...
        % <dxi, dxi>
        innerProdTemp(1:(p*(p+1)),1:(p*(p+1))) = xietaBasisXi*(xietaWeightsg11gMatrix*xietaBasisXi');

        % <dxi, deta>
        if ~isempty(g12)
            innerProdTemp((p*(p+1)+1):end,1:(p*(p+1))) = (xietaBasisXi*(xietaWeightsg12gMatrix*xietaBasisEta'))';
            innerProdTemp(1:(p*(p+1)),(p*(p+1)+1):end) = (xietaBasisEta*(xietaWeightsg12gMatrix*xietaBasisXi'))';
        end

        % <deta, deta>
        innerProdTemp((p*(p+1)+1):end,(p*(p+1)+1):end) = xietaBasisEta*(xietaWeightsg22gMatrix*xietaBasisEta');
