function gn = GlobalNumberingZeroFormPrimalPeriodic(n,p,bcFlag)
...
% 
%             for k=1:(n(1)-1)
%                 gn(rightEdgeNodesInternalNumber,((k-1)*n(2)+1):k*n(2)) = reshape(nodesgn(((k-1)*(p-1)*n(2)+1):(k*n(2)*(p-1))),[p-1 n(2)]);
%                 gn(leftEdgeNodesInternalNumber,(k*n(2)+1):(k+1)*n(2)) = reshape(nodesgn(((k-1)*(p-1)*n(2)+1):(k*n(2)*(p-1))),[p-1 n(2)]);
%             end
%         end
% 
...
% 
%             for k=1:(n(2)-1)
%                 gn(topEdgeNodesInternalNumber,k:n(2):(n(1)*n(2))) = reshape(nodesgn(((k-1)*(p-1)*n(1)+1):(k*n(1)*(p-1))),[p-1 n(1)]);
%                 gn(bottomEdgeNodesInternalNumber,(k:n(2):(n(1)*n(2)))+1) = reshape(nodesgn(((k-1)*(p-1)*n(1)+1):(k*n(1)*(p-1))),[p-1 n(1)]);
%             end
%         end
%     end
...
%         gn(bottomEdgeNodesInternalNumber,1:n(2):((n(1)-1)*n(2))+1) = reshape(nodesgn,[(p-1) n(1)]); % lower boundary
%         
%         if ~bcFlag(1)
%             nodesgn = uint32(nodesgn(end)+1):uint32(nodesgn(end)+n(1)*(p-1));% upper boundary
%         end
%         gn(topEdgeNodesInternalNumber,(1:n(2):((n(1)-1)*n(2))+1)+n(2)-1) = reshape(nodesgn,[(p-1) n(1)]); % upper boundary
% 
%         nodesgn = uint32(nodesgn(end)+1):uint32(nodesgn(end)+n(2)*(p-1));% left boundary
%         gn(leftEdgeNodesInternalNumber,1:n(2)) = reshape(nodesgn,[(p-1) n(2)]); % left boundary
%         
%         if ~bcFlag(2)
%             nodesgn = uint32(nodesgn(end)+1):uint32(nodesgn(end)+n(2)*(p-1));% right boundary
%         end
%         gn(rightEdgeNodesInternalNumber,((n(1)-1)*n(2)+1):(n(1)*n(2))) = reshape(nodesgn,[(p-1) n(2)]); % right boundary
%     end
%     
%     gn = gn';
        if length(n)>2
