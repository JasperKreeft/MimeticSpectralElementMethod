function gn = GlobalNumberingOneFormPrimalPeriodic(n,p,bcFlag)
% GlobalNumberingOneFormPrimalPeriodic computes the global numbering of 1-form
% degrees of freedom on a  primal mesh. Enforcing periodic boundary
% conditions.
%
%   gn = GlobalNumberingOneFormPrimalPeriodic(n, p)
%
%   Where:
...
%       bcFlag :: is a flag that states if periodic boundary conditions should
%                 be used for the numbering: true => boundary conditions
%              :: [BottomTopFlag; RightLeftFlag]
%
...
%--------------------------------------------------------------------------
    ...
    gn = zeros(2*nElementEdges,nElements);

    % number internal edges

    % first select the internal edges of the elements
    internalEdgesColumns = zeros(nElementInternalEdges,1);
    for i=1:p
        for j=2:p
...
%     end
    ...
    if n(2)~=1
        if nInternalElements >= 1
            edgesgn = (edgesgn(end)+1):(edgesgn(end)+p*n(1)*(n(2)-1));
        else
            edgesgn = (nInternalEdges+1):(nInternalEdges+p*n(1)*(n(2)-1));
        end

        for k=1:(n(2)-1)
            gn(topEdgeEdgesInternalNumber,k:n(2):(n(1)*n(2))) = reshape(edgesgn(((k-1)*p*n(1)+1):(k*n(1)*p)),[p n(1)]);
            gn(bottomEdgeEdgesInternalNumber,(k:n(2):(n(1)*n(2)))+1) = reshape(edgesgn(((k-1)*p*n(1)+1):(k*n(1)*p)),[p n(1)]);
        end
    end
...
%     topEdgeEdgesInternalNumber = bottomEdgeEdgesInternalNumber + p;
    ...
    if n(1)~=1
        if nInternalElements >= 1
            edgesgn = (edgesgn(end)+1):(edgesgn(end)+p*n(2)*(n(1)-1));
        else
            edgesgn = (nInternalEdges+1+lastNumber):(nInternalEdges+p*n(2)*(n(1)-1)+lastNumber);
        end

        for k=1:(n(1)-1)
            gn(rightEdgeEdgesInternalNumber+nElementEdges,((k-1)*n(2)+1):k*n(2)) = reshape(edgesgn(((k-1)*p*n(2)+1):(k*n(2)*p)),[p n(2)]);
            gn(leftEdgeEdgesInternalNumber+nElementEdges,(k*n(2)+1):(k+1)*n(2)) = reshape(edgesgn(((k-1)*p*n(2)+1):(k*n(2)*p)),[p n(2)]);
        end
    end
...
%--------------------------------------------------------------------------
    ...
    edgesgn =  (edgesgn(end)+1):(edgesgn(end)+n(1)*p);% lower boundary
    gn(bottomEdgeEdgesInternalNumber,1:n(2):((n(1)-1)*n(2))+1) = reshape(edgesgn,[p n(1)]); % lower boundary
    
    if ~bcFlag(1)
        edgesgn = (edgesgn(end)+1):(edgesgn(end)+n(1)*p);% upper boundary
    end
    gn(topEdgeEdgesInternalNumber,(1:n(2):((n(1)-1)*n(2))+1)+n(2)-1) = reshape(edgesgn,[p n(1)]); % upper boundary
    
    edgesgn = (edgesgn(end)+1):(edgesgn(end)+n(2)*p);% left boundary
    gn(leftEdgeEdgesInternalNumber+nElementEdges,1:n(2)) = reshape(edgesgn,[p n(2)]); % left boundary
    
    if ~bcFlag(2)
        edgesgn = (edgesgn(end)+1):(edgesgn(end)+n(2)*p);% right boundary
    end
    gn(rightEdgeEdgesInternalNumber+nElementEdges,((n(1)-1)*n(2)+1):(n(1)*n(2))) = reshape(edgesgn,[p n(2)]); % right boundary
