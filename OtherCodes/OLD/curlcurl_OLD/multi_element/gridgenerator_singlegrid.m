...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                         %
% Grid generator for curl-curl problem on [0,pi]^2 square                 %
%                                                                         %
...
for r=1:numRows
    for c=1:numColumns
        ...
        dXdXib  = pi/2*(1+pi*cc*cos(pi*Xib).*sin(pi*Etab));
        dXdEtab = pi^2/2*cc*sin(pi*Xib).*cos(pi*Etab);
        dYdXib  = pi^2/2*cc*cos(pi*Xib).*sin(pi*Etab);
        dYdEtab = pi/2*(1+pi*cc*sin(pi*Xib).*cos(pi*Etab));
...
% figure
% mesh(X,Y,zeros(size(X)),'EdgeColor','black')
% view([0 0 1])
% axis square off
