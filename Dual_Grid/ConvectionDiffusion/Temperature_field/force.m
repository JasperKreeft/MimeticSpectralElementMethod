...
xiLR  = ( ((2*c-1)/numColumns-1)+xiGLL/numColumns );
etaAB = ( ((2*r-1)/numRows-1)+etaGLL/numRows );
...
for i=1:N
    for j=1:N
        ...
        dxibdxi   = (xiLR(i+1)-xiLR(i))/2*ones(n);
        dxibdeta  = zeros(n);
        detabdxi  = zeros(n);
        detabdeta = (etaAB(j+1)-etaAB(j))/2*ones(n);
