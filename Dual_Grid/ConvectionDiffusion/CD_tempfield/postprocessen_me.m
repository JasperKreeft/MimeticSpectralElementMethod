...
for r=1:numRows-1
    for c=1:numColumns-1
        ...
        if c==1
            if BC(1,1)==1
                kk = k(end)+(1:(1+(BC(1,1)==1)):(1+(BC(1,1)==1))*N);
                PHI(1,(1:N)+1+(r-1)*(N+1)) = reshape(phi_in(kk),1,N);
            end
            k = k(end)+((1+(BC(1,1)==1)):(1+(BC(1,1)==1)):(1+(BC(1,1)==1))*N);
            PHI(N+2,(1:N)+1+(r-1)*(N+1)) = reshape(phi_in(k),1,N);
        else        
            k = k(end)+(1:N);
        ...
        if r==1
            if BC(1,3)==1
                kk = k(N)+(1:(1+(BC(1,3)==1)):(1+(BC(1,3)==1))*N);
                PHI(1+(c-1)*(N+1)+(1:N),1) = reshape(phi_in(kk),N,1);
            end
            k = k(N)+((1+(BC(1,3)==1)):(1+(BC(1,3)==1)):(1+(BC(1,3)==1))*N);
            PHI(1+(c-1)*(N+1)+(1:N),N+2) = reshape(phi_in(k),N,1);
        else
            k = k(N)+(1:N);
    ...
    if r==1
        if BC(1,3)==1
            kk = k(N)+(1:(1+(BC(1,3)==1)):(1+(BC(1,3)==1))*N);
            PHI(1+(numColumns-1)*(N+1)+(1:N),1) = reshape(phi_in(kk),N,1);
        end
        k = k(N)+((1+(BC(1,3)==1)):(1+(BC(1,3)==1)):(1+(BC(1,3)==1))*N);
...
for c=1:numColumns-1
    ...
    if c==1
        if BC(1,1)==1
            kk = k(N2)+(1:(1+(BC(1,1)==1)):(1+(BC(1,1)==1))*N);
            PHI(1,(1:N)+1+(numRows-1)*(N+1)) = reshape(phi_in(kk),1,N);
        end
    ...
    k = k(N)+BC(1,4)*(1:N);
    PHI((c-1)*(N+1)+1+(1:N),1+numRows*(N+1)) = reshape(phi_in(k),1,N);
end
% Last Element
k = k(N)+(1:N2);
...
k = k(N)+BC(1,4)*(1:N);
PHI((1:N)+1+(numColumns-1)*(N+1),1+numRows*(N+1)) = reshape(phi_in(k),N,1);
...
if BC(2,2)==1
    PHI(numColumns*(N+1)+1,ind) = phi_bc(BC(2,1)*N*numRows+(1:N*numRows)); % phi_rightbc;
end

ind = 1:numColumns*(N+1)+1; ind(1:N+1:numColumns*(N+1)+1) = [];
if BC(2,3)==1
    PHI(ind,1) = phi_bc((BC(2,1)+BC(2,2))*N*numRows+(1:N*numColumns)); %phi_lowerbc;
end
if BC(2,4)==1
    PHI(ind,numRows*(N+1)+1) = phi_bc((BC(2,1)+BC(2,2))*N*numRows+BC(2,3)*N*numColumns+(1:N*numColumns)); % phi_upperbc;
end
