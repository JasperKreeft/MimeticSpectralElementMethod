...
if length(NrCellRange)>1
    ...
    hold on
    semilogy(NrCellRange,abs(errorL2(NrCellRange)-errorL2_interp(NrCellRange)),'g')
    xlim([NrCellRange(1) N])
    xlabel('N')
elseif length(HconvRange)>1
    if exist('errorL2_interp','var')
    loglog(2./(HconvRange),errorL2_interp(NrCellRange),'--or','markerface','r')
    hold on
    end
    loglog(2./(HconvRange),errorL2(NrCellRange),'-^b','markerface','b')
    hold on
    grid on
%     xlim([])
    xlabel('h')
