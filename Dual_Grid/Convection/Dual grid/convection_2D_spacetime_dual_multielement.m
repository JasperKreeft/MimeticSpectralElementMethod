...
globalnr = zeros(numColumns*(N+1)+1,numRows*(N+1)+1);
for r=1:numRows
    for c=1:numColumns
        ...
        globalnr((c-1)*(N+1)+1+(1:N),(r-1)*(N+1)+1+(1:N)) = localnr+max_previous_element;

        if c==1
            globalnr(1,(r-1)*(N+1)+1+(1:N)) = (1:2:2*N)+N2+...
                          (r-1)*(numColumns*(N2+2*N)+N)+(r>1)*N*numColumns;

            globalnr(N+2,(r-1)*(N+1)+1+(1:N)) = (2:2:2*N)+N2+...
                          (r-1)*(numColumns*(N2+2*N)+N)+(r>1)*N*numColumns;
        else
            globalnr(c*(N+1)+1,(r-1)*(N+1)+1+(1:N)) = (1:N)+N2+(c-1)*(N2+2*N)+...
                          (r==1)*(c-1)*N+...
                          N*(c>1)+...
                          (r-1)*(numColumns*(N2+2*N)+N)+(r>1)*N*numColumns;
        end
        if r==1
            ...
            globalnr((c-1)*(N+1)+1+(1:N),r*(N+1)+1) = max_previous_element+N2+(1+(c==1))*N+(1:N);
        end
    end
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%       
        ...
        C2 = speye(numRows*numColumns*(N2+4*N));
        for r=numRows:-1:1
