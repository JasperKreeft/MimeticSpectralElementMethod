function []=numbering_FicheraCorner()
...
% el8(:,1,:) = el6(:,N+1,:);
% el8(1,:,:) = el7(N+1,:,:);
% el8(2:N+1,2:N+1,2:N+1) = reshape(nr_0+(1:N^3),N,N,N);
% globalnr_0(:,8) = reshape(el8,(N+1)^3,1);
% nr_0 = max(globalnr_0(:,8));

%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

globalnr_1x = zeros(N*(N+1)^2,7);
globalnr_1y = zeros(N*(N+1)^2,7);
globalnr_1z = zeros(N*(N+1)^2,7);
...
nr_1= max(globalnr_1x(:,7));

% % 8th element
% el8z = el0z;
% el8z(:,:,1) = el6z(:,:,N+1);
% el8z(:,1,:) = el7z(:,N+1,:);
% el8z(:,2:N+1,2:N+1) = reshape(nr_1+(1:N^3),N,N,N);
...
globalnr_2x = zeros(N^2*(N+1),7);
globalnr_2y = zeros(N^2*(N+1),7);
globalnr_2z = zeros(N^2*(N+1),7);
...
nr_2 = max(globalnr_2z(:,7));


% % 8th element
% el8x = el0x;
% el8x(1,:,:) = el7x(N+1,:,:);
% el8x(2:N+1,:,:) = nr_2+reshape(1:N^3,N,N,N);
...
globalnr_3 = zeros(N^3,7);
for i=1:7
    globalnr_3(:,i) = (i-1)*N^3+(1:N^3);
end
nr_3 = globalnr_3(N^3,7);


% % 8th element
% globalnr_3(:,8) = 7*N^3+(1:N^3);
