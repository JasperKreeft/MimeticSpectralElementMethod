...
%     UbcL = -ux.*dxdeta - uy.*dydeta;  % Stokes
UbcL = ux.*dydeta - uy.*dxdeta;  % Scalar Poisson
...
if bc(2) == 1
    ...
    dydeta = Mesh.dYdEta(ind1,ind2);
    [ux uy] = exact_solution(Mesh.X(ind1,ind2),Mesh.Y(ind1,ind2),FunctionType,'one');
%     UbcR = - ux.*dxdeta - uy.*dydeta;
UbcR = ux.*dydeta - uy.*dxdeta;
    ind3 = 1:size(ind2,2)-1;
