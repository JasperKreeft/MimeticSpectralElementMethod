...
for r=1:numRows
    ...
    yrc = (etabAB(r+1)+etabAB(r))/2+(etabAB(r+1)-etabAB(r))/2*etaG;
    ...
    for c=1:numColumns
        ...
        xrc = (xibLR(c+1)+xibLR(c))/2+(xibLR(c+1)-xibLR(c))/2*xiG;

        XXrc = Xrc+cc*sin(pi*Xrc).*sin(pi*Yrc);
        YYrc = Yrc+cc*sin(pi*Xrc).*sin(pi*Yrc);
        
        xxrc = xrc'*ones(1,N)+cc*sin(pi*xrc)'*sin(pi*yrc);
        yyrc = ones(N,1)*yrc +cc*sin(pi*xrc)'*sin(pi*yrc);
        
        phi_ex(:,:,rc) = sin(m*pi*XXrc).*sin(m*pi*YYrc);
        pphi(:,:,rc)       = hG'*PHI((c-1)*(N+1)+1+(1:N),(r-1)*(N+1)+1+(1:N))*hG;
        phi_exact          = sin(m*pi*xxrc).*sin(m*pi*yyrc);
        phi_interp(:,:,rc) = hG'*phi_exact*hG;
...
% figure
% for r=1:numRows
%     yrc = (etabAB(r+1)+etabAB(r))/2+(etabAB(r+1)-etabAB(r))/2*yy;
%     for c=1:numColumns
%         xrc = (xibLR(c+1)+xibLR(c))/2+(xibLR(c+1)-xibLR(c))/2*xx;
%         rc = c+(r-1)*numColumns;
%         
%         subplot(2,3,1)
