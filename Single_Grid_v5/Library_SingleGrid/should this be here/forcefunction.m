...
ind = c+(r-1)*(numColumns+1);
xiLR  = (xibLR(ind)+xibLR(ind+1))/2+(xibLR(ind+1)-xibLR(ind))/2*xieg';
etaAB = (etabAB(ind)+etabAB(ind+numColumns+1))/2+(etabAB(ind+numColumns+1)-etabAB(ind))/2*eta;
...
for i=1:N+1
    for j=1:N
        ...
        fx = exact_solution(xx,yy,FunctionType,'force');
...
ind = c+(r-1)*(numColumns+1);
xiLR  = (xibLR(ind)+xibLR(ind+1))/2+(xibLR(ind+1)-xibLR(ind))/2*xi';
etaAB = (etabAB(ind)+etabAB(ind+numColumns+1))/2+(etabAB(ind+numColumns+1)-etabAB(ind))/2*etaeg;
...
for i=1:N
    for j=1:N+1
        ...
        fy = exact_solution(xx,yy,FunctionType,'force');
