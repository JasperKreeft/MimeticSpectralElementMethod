function varargout = initialforms(form,varargin)
...
x0 = 0;
y0 = -1/2;
F = exp(-((X-x0).^2+(Y-y0).^2)/(2*lambda^2));
...
%         [fx,fy] = exact_solution(xx,yy,FunctionType,'force');
        ...
        x0 =  1/2;
        y0 = -1/2;
        fx = -exp(-((xx-x0).^2+(yy-y0).^2)/(2*lambda^2));%
%         fx = ones(size(xx));%
        x0 = -1/2;
        y0 =  1/2;
        fy = exp(-((xx-x0).^2+(yy-y0).^2)/(2*lambda^2));%
%         fy = ones(size(xx));%
...
%         f = exact_solution(xx,yy,FunctionType,'force');
        ...
        x0 = -1/2;
        y0 = 0;
        f = exp(-((xx-x0).^2+(yy-y0).^2)/(2*lambda^2));
